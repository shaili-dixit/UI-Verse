<template>
<html data-theme="light">

  <div class="main-content">

    <!-- TOPBAR -->
    <header class="topbar">
      <div class="search-box">
        <span class="brand-icon">⬡</span>
        <span class="brand-text">UIverse</span>
        <i class="fa-solid fa-magnifying-glass"></i>
        <input type="text" placeholder="Search components...">
      </div>
      <div class="topbar-actions">
        
        <button class="add-btn"><i class="fa-solid fa-plus"></i> Add Component</button>
        <button class="collection-btn">My Collection</button>
        <button class="theme-btn"><i class="fa-solid fa-moon"></i></button>
      </div>
    </header>

    <!-- HERO -->
    <section class="hero">
      <div class="hero-left">
        <div class="breadcrumb">Home &gt; Checkboxes</div>
        <h1>Checkbox Components</h1>
        <p>
          Modern reusable checkbox UI elements with responsive layouts,
          interactive styling, and smooth check/uncheck animations using HTML, CSS, and Vanilla JavaScript.
        </p>
        <div class="hero-tags">
          <span><i class="fa-solid fa-layer-group"></i> 5 Components</span>
          <span><i class="fa-solid fa-code"></i> Pure HTML/CSS</span>
          <span><i class="fa-solid fa-mobile-screen"></i> Responsive</span>
        </div>
      </div>
      <div class="hero-preview">
        <div class="hero-checkbox-demo">
          <div class="hero-cb checked"><i class="fa-solid fa-check"></i></div>
          <div class="hero-cb"><i class="fa-solid fa-check"></i></div>
          <div class="hero-cb checked"><i class="fa-solid fa-check"></i></div>
        </div>
      </div>
    </section>

    <!-- ================= SIDEBAR ================= -->
    <aside class="sidebar" id="sidebar">
      <div class="sidebar-brand">
        <span class="brand-icon">⬡</span>
        <span class="brand-text">UIverse</span>
      </div>
      <nav class="sidebar-nav">
        <ul>
          <li><a href="index.html"><i class="fa-solid fa-house"></i><span>Home</span></a></li>
          <li><a href="button.html"><i class="fa-solid fa-hand-pointer"></i><span>Buttons</span></a></li>
          <li>
            <a href="dropdown-components.html">
              <i class="fa-solid fa-caret-down"></i>
              <span>Dropdowns</span>
            </a>
          </li>
          <li>
            <a href="profile-components.html">
              <i class="fa-solid fa-user"></i>
              <span>Profiles</span>
            </a>
          </li>
          <li><a href="navbar.html"><i class="fa-solid fa-bars"></i><span>Navbar</span></a></li>
          <li><a href="cards.html"><i class="fa-solid fa-table-cells-large"></i><span>Cards</span></a></li>
          <li><a href="flipcards.html"><i class="fa-solid fa-clone"></i><span>3D Cards</span></a></li>
          <li><a href="inputs.html"><i class="fa-solid fa-keyboard"></i><span>Inputs</span></a></li>
          <li><a href="forms.html"><i class="fa-brands fa-wpforms"></i><span>Forms</span></a></li>
          <li><a href="badges.html"><i class="fa-solid fa-award"></i><span>Badges</span></a></li>
          <li><a href="blog.html"><i class="fa-solid fa-blog"></i><span>Blog</span></a></li>
          <li><a href="article.html"><i class="fa-solid fa-newspaper"></i><span>Articles</span></a></li>
          <li><a href="alerts.html"><i class="fa-solid fa-triangle-exclamation"></i><span>Alerts</span></a></li>
          <li><a href="color.html"><i class="fa-solid fa-palette"></i><span>Colors</span></a></li>
          <li><a href="charts.html"><i class="fa-solid fa-chart-pie"></i><span>Charts</span></a></li>
          <li><a href="dashboard.html"><i class="fa-solid fa-gauge-high"></i><span>Dashboard</span></a></li>
          <li><a href="div.html"><i class="fa-solid fa-square"></i><span>DIV</span></a></li>
          <li><a href="widgets.html"><i class="fa-solid fa-layer-group"></i><span>Widgets</span></a></li>
          <li><a href="search.html"><i class="fa-solid fa-magnifying-glass"></i><span>Search Bars</span></a></li>
          <li><a href="hover.html"><i class="fa-solid fa-wand-magic-sparkles"></i><span>Hover Effects</span></a></li>
          <li><a href="error.html"><i class="fa-solid fa-circle-exclamation"></i><span>Error Pages</span></a></li>
          <li><a href="ecommerce.html"><i class="fa-solid fa-cart-shopping"></i><span>E-commerce</span></a></li>
          <li><a href="files.html"><i class="fa-solid fa-file-arrow-up"></i><span>Drag & Drop</span></a></li>
          <li><a href="hero.html"><i class="fa-solid fa-star"></i><span>Hero Sections</span></a></li>
          <li><a href="loaders.html"><i class="fa-solid fa-spinner"></i><span>Loaders</span></a></li>
          <li><a href="timeline.html"><i class="fa-solid fa-clock-rotate-left"></i><span>Timeline</span></a></li>
          <li><a href="map.html"><i class="fa-solid fa-map-location-dot"></i><span>Maps</span></a></li>
          <li><a href="menu.html"><i class="fa-solid fa-bars-staggered"></i><span>Menu</span></a></li>
          <li><a href="pricing.html"><i class="fa-solid fa-tags"></i><span>Pricing</span></a></li>
          <li>
            <a href="subscription.html">
              <i class="fa-solid fa-credit-card"></i>
              <span>Subscription</span>
            </a>
          </li>
          <li>
            <a href="auth.html">
              <i class="fa-solid fa-user-shield"></i>
              <span>Authentication</span>
            </a>
          </li>
          <li>
            <a href="recovery.html">
              <i class="fa-solid fa-key" aria-hidden="true"></i>
              <span>Password Recovery</span>
            </a>
          </li>
          <li><a href="section.html"><i class="fa-solid fa-rectangle-list"></i><span>Section</span></a></li>
          <li><a href="span.html"><i class="fa-solid fa-code"></i><span>Span</span></a></li>
          <li><a href="table.html"><i class="fa-solid fa-table"></i><span>Table</span></a></li>
          <li><a href="tabs.html"><i class="fa-solid fa-table-columns"></i><span>Tabs</span></a></li>
          <li><a href="terms.html"><i class="fa-solid fa-file-contract"></i><span>Terms</span></a></li>
          <li><a href="testimonials.html"><i class="fa-solid fa-comments"></i><span>Testimonials</span></a></li>
          <li><a href="toggles.html"><i class="fa-solid fa-toggle-on"></i><span>Toggle</span></a></li>
          <li><a href="radiobutton.html"><i class="fa-solid fa-circle-dot"></i><span>Radio Buttons</span></a></li>
          <li class="active"><a href="checkbox.html"><i class="fa-solid fa-square-check"></i><span>Checkboxes</span></a>
          </li>
          <li><a href="notifications-premium.html"><i class="fa-solid fa-bell"></i><span>Notifications V2</span></a>
          </li>
          <li><a href="step-indicators.html"><i class="fa-solid fa-list-check"></i><span>Steppers</span></a></li>
          <li><a href="progress-premium.html"><i class="fa-solid fa-bars-progress"></i><span>Progress V2</span></a></li>
          <li><a href="ratings-premium.html"><i class="fa-solid fa-star"></i><span>Ratings V2</span></a></li>
          <li><a href="filters-premium.html"><i class="fa-solid fa-sliders"></i><span>Filters V2</span></a></li>
          <li><a href="about.html"><i class="fa-solid fa-circle-info"></i><span>About</span></a></li>
          <li><a href="documentation.html"><i class="fa-solid fa-book"></i><span>Documentation</span></a></li>
          <li><a href="faq.html"><i class="fa-solid fa-circle-question"></i><span>Faq</span></a></li>
          <li><a href="contact.html"><i class="fa-regular fa-envelope"></i><span>Contact Us</span></a></li>
        </ul>
      </nav>
      <div class="sidebar-footer">
        <a href="#" title="GitHub"><i class="fab fa-github"></i></a>
        <a href="#" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
        <a href="#" title="Twitter"><i class="fab fa-x-twitter"></i></a>
      </div>
    </aside>

    <!-- FILTERS -->
    <div class="filters">
      <button class="active">All</button>
      <button>Minimal</button>
      <button>Gradient</button>
      <button>Glass</button>
      <button>Card</button>
    </div>

    <!-- COMPONENT GRID -->
    <section class="checkbox-grid">

      <!-- ============ 1: MINIMAL CHECKBOX ============ -->
      <div class="component-card">
        <div class="card-preview">
          <div class="minimal-cb-group">
            <label class="minimal-cb">
              <input type="checkbox" checked>
              <span class="minimal-box"><i class="fa-solid fa-check"></i></span>
              <span class="minimal-text">Enable notifications</span>
            </label>
            <label class="minimal-cb">
              <input type="checkbox">
              <span class="minimal-box"><i class="fa-solid fa-check"></i></span>
              <span class="minimal-text">Auto-save drafts</span>
            </label>
            <label class="minimal-cb">
              <input type="checkbox">
              <span class="minimal-box"><i class="fa-solid fa-check"></i></span>
              <span class="minimal-text">Dark mode by default</span>
            </label>
          </div>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Minimal Checkbox</h3>
            <span>Essential</span>
          </div>
          <p>Clean checkbox with smooth scale-in checkmark animation and subtle border transitions.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb1')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb1')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb1" class="code-block" style="display:none;"><code>&lt;label class="minimal-cb"&gt;
  &lt;input type="checkbox" checked&gt;
  &lt;span class="minimal-box"&gt;&lt;i class="fa-solid fa-check"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span&gt;Enable notifications&lt;/span&gt;
&lt;/label&gt;

.minimal-cb input:checked ~ .minimal-box {
  background: #6366f1;
  border-color: #6366f1;
}
.minimal-cb input:checked ~ .minimal-box i {
  transform: scale(1);
  opacity: 1;
}</code></pre>
      </div>

      <!-- ============ 2: GRADIENT CHECKBOX ============ -->
      <div class="component-card">
        <div class="card-preview">
          <div class="gradient-cb-group">
            <label class="gradient-cb">
              <input type="checkbox" checked>
              <span class="gradient-box"><i class="fa-solid fa-check"></i></span>
              <span class="gradient-text">I agree to Terms of Service</span>
            </label>
            <label class="gradient-cb">
              <input type="checkbox">
              <span class="gradient-box"><i class="fa-solid fa-check"></i></span>
              <span class="gradient-text">Subscribe to newsletter</span>
            </label>
            <label class="gradient-cb">
              <input type="checkbox" checked>
              <span class="gradient-box"><i class="fa-solid fa-check"></i></span>
              <span class="gradient-text">Remember my preferences</span>
            </label>
          </div>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Gradient Checkbox</h3>
            <span>Trending</span>
          </div>
          <p>Vibrant gradient-filled checkbox with animated border glow on check and smooth colour transitions.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb2')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb2')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb2" class="code-block" style="display:none;"><code>&lt;label class="gradient-cb"&gt;
  &lt;input type="checkbox" checked&gt;
  &lt;span class="gradient-box"&gt;&lt;i class="fa-solid fa-check"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span&gt;I agree to Terms&lt;/span&gt;
&lt;/label&gt;

.gradient-cb input:checked ~ .gradient-box {
  background: linear-gradient(135deg, #7c3aed, #06b6d4);
  border-color: transparent;
  box-shadow: 0 0 12px rgba(124,58,237,0.35);
}</code></pre>
      </div>

      <!-- ============ 3: GLASSMORPHISM CHECKBOX ============ -->
      <div class="component-card">
        <div class="card-preview dark-preview">
          <div class="glass-cb-group">
            <label class="glass-cb">
              <input type="checkbox" checked>
              <span class="glass-box"><i class="fa-solid fa-check"></i></span>
              <span class="glass-text"><i class="fa-solid fa-lock"></i> Two-Factor Auth</span>
            </label>
            <label class="glass-cb">
              <input type="checkbox">
              <span class="glass-box"><i class="fa-solid fa-check"></i></span>
              <span class="glass-text"><i class="fa-solid fa-eye-slash"></i> Incognito Mode</span>
            </label>
            <label class="glass-cb">
              <input type="checkbox">
              <span class="glass-box"><i class="fa-solid fa-fingerprint"></i></span>
              <span class="glass-text"><i class="fa-solid fa-fingerprint"></i> Biometric Login</span>
            </label>
          </div>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Glassmorphism Checkbox</h3>
            <span>Premium</span>
          </div>
          <p>Frosted translucent checkbox cards with backdrop blur, glowing active states, and icon labels.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb3')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb3')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb3" class="code-block" style="display:none;"><code>&lt;label class="glass-cb"&gt;
  &lt;input type="checkbox" checked&gt;
  &lt;span class="glass-box"&gt;&lt;i class="fa-solid fa-check"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span class="glass-text"&gt;Two-Factor Auth&lt;/span&gt;
&lt;/label&gt;

.glass-cb input:checked ~ .glass-box {
  background: rgba(99,102,241,0.35);
  border-color: rgba(99,102,241,0.6);
  box-shadow: 0 0 14px rgba(99,102,241,0.3);
}</code></pre>
      </div>

      <!-- ============ 4: ANIMATED CHECKBOX TOGGLE ============ -->
      <div class="component-card">
        <div class="card-preview">
          <div class="anim-toggle-group">
            <label class="anim-toggle">
              <input type="checkbox" checked>
              <span class="anim-track">
                <span class="anim-thumb"></span>
              </span>
              <span class="anim-label">Airplane Mode</span>
            </label>
            <label class="anim-toggle">
              <input type="checkbox">
              <span class="anim-track">
                <span class="anim-thumb"></span>
              </span>
              <span class="anim-label">Wi-Fi Sync</span>
            </label>
            <label class="anim-toggle">
              <input type="checkbox" checked>
              <span class="anim-track">
                <span class="anim-thumb"></span>
              </span>
              <span class="anim-label">Bluetooth</span>
            </label>
          </div>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Animated Checkbox Toggle</h3>
            <span>Popular</span>
          </div>
          <p>iOS-inspired toggle switch using a checkbox with a sliding thumb and smooth colour fill transition.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb4')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb4')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb4" class="code-block" style="display:none;"><code>&lt;label class="anim-toggle"&gt;
  &lt;input type="checkbox" checked&gt;
  &lt;span class="anim-track"&gt;
    &lt;span class="anim-thumb"&gt;&lt;/span&gt;
  &lt;/span&gt;
  &lt;span&gt;Airplane Mode&lt;/span&gt;
&lt;/label&gt;

.anim-toggle input:checked ~ .anim-track {
  background: #6366f1;
}
.anim-toggle input:checked ~ .anim-track .anim-thumb {
  transform: translateX(20px);
}</code></pre>
      </div>

      <!-- ============ 5: TASK LIST CHECKBOX CARD ============ -->
      <div class="component-card">
        <div class="card-preview dark-preview">
          <div class="task-card">
            <div class="task-header">
              <h4>Today's Tasks</h4>
              <span class="task-count">2/4</span>
            </div>
            <label class="task-item">
              <input type="checkbox" checked>
              <span class="task-box"><i class="fa-solid fa-check"></i></span>
              <span class="task-text done">Design landing page</span>
            </label>
            <label class="task-item">
              <input type="checkbox" checked>
              <span class="task-box"><i class="fa-solid fa-check"></i></span>
              <span class="task-text done">Review pull requests</span>
            </label>
            <label class="task-item">
              <input type="checkbox">
              <span class="task-box"><i class="fa-solid fa-check"></i></span>
              <span class="task-text">Write unit tests</span>
            </label>
            <label class="task-item">
              <input type="checkbox">
              <span class="task-box"><i class="fa-solid fa-check"></i></span>
              <span class="task-text">Deploy to staging</span>
            </label>
          </div>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Task List Checkbox Card</h3>
            <span>New</span>
          </div>
          <p>Complete task list card with strikethrough animation, progress counter, and interactive checkbox states.
          </p>
          <div class="card-actions">
            <button onclick="toggleCode('cb5')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb5')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb5" class="code-block" style="display:none;"><code>&lt;div class="task-card"&gt;
  &lt;div class="task-header"&gt;
    &lt;h4&gt;Today's Tasks&lt;/h4&gt;
    &lt;span class="task-count"&gt;2/4&lt;/span&gt;
  &lt;/div&gt;
  &lt;label class="task-item"&gt;
    &lt;input type="checkbox" checked&gt;
    &lt;span class="task-box"&gt;&lt;i class="fa-solid fa-check"&gt;&lt;/i&gt;&lt;/span&gt;
    &lt;span class="task-text done"&gt;Design landing page&lt;/span&gt;
  &lt;/label&gt;
&lt;/div&gt;

.task-item input:checked ~ .task-text {
  text-decoration: line-through;
  color: #475569;
}</code></pre>
      </div>
      <div class="component-card">
        <div class="card-preview dark-preview">
          <label class="cyber-cb">
            <input type="checkbox">
            <span class="cyber-box">
              <i class="fa-solid fa-xmark"></i>
            </span>
            <span class="cyber-text">OVERRIDE</span>
          </label>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Cyberpunk Glitch Checkbox</h3>
            <span>Premium</span>
          </div>
          <p>A futuristic glitchy checkbox with a neon crossed-out checkmark and jagged styling.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb6')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb6')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb6" class="code-block" style="display:none;"><code>&lt;label class="cyber-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;span class="cyber-box"&gt;&lt;i class="fa-solid fa-xmark"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span class="cyber-text"&gt;OVERRIDE&lt;/span&gt;
&lt;/label&gt;</code></pre>
      </div>

      <div class="component-card">
        <div class="card-preview" style="background:#e0e5ec;">
          <label class="neumorphic-cb">
            <input type="checkbox">
            <span class="neu-box"><i class="fa-solid fa-power-off"></i></span>
            <span class="neu-text">Power Up</span>
          </label>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Neumorphic Checkbox</h3>
            <span>Popular</span>
          </div>
          <p>Soft UI neumorphic checkbox with extruded shadows and an inset pressed state.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb7')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb7')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb7" class="code-block" style="display:none;"><code>&lt;label class="neumorphic-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;span class="neu-box"&gt;&lt;i class="fa-solid fa-power-off"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span class="neu-text"&gt;Power Up&lt;/span&gt;
&lt;/label&gt;</code></pre>
      </div>

      <div class="component-card">
        <div class="card-preview">
          <label class="heart-cb">
            <input type="checkbox">
            <span class="heart-icon"><i class="fa-solid fa-heart"></i></span>
          </label>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Heart Favorite Checkbox</h3>
            <span>Essential</span>
          </div>
          <p>A simple animated heart for "Like" or "Favorite" interactions using standard checkbox inputs.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb8')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb8')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb8" class="code-block" style="display:none;"><code>&lt;label class="heart-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;span class="heart-icon"&gt;&lt;i class="fa-solid fa-heart"&gt;&lt;/i&gt;&lt;/span&gt;
&lt;/label&gt;</code></pre>
      </div>

      <div class="component-card">
        <div class="card-preview dark-preview">
          <label class="plan-cb">
            <input type="checkbox">
            <div class="plan-card">
              <div class="plan-header">
                <span class="plan-title">Pro Tier</span>
                <span class="plan-box"><i class="fa-solid fa-check"></i></span>
              </div>
              <div class="plan-price">$19<span>/mo</span></div>
              <p>Access to all premium features and components.</p>
            </div>
          </label>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Subscription Plan Checkbox</h3>
            <span>Trending</span>
          </div>
          <p>A selectable plan card built purely with a hidden checkbox, ideal for pricing tables.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb9')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb9')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb9" class="code-block" style="display:none;"><code>&lt;label class="plan-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;div class="plan-card"&gt;
    &lt;div class="plan-header"&gt;
      &lt;span class="plan-title"&gt;Pro Tier&lt;/span&gt;
      &lt;span class="plan-box"&gt;&lt;i class="fa-solid fa-check"&gt;&lt;/i&gt;&lt;/span&gt;
    &lt;/div&gt;
    &lt;div class="plan-price"&gt;$19&lt;span&gt;/mo&lt;/span&gt;&lt;/div&gt;
    &lt;p&gt;Premium features&lt;/p&gt;
  &lt;/div&gt;
&lt;/label&gt;</code></pre>
      </div>

      <div class="component-card">
        <div class="card-preview">
          <label class="terms-cb">
            <input type="checkbox">
            <span class="terms-box"><i class="fa-solid fa-check"></i></span>
            <span class="terms-text">I agree to the <a href="#" onclick="event.preventDefault();">Terms</a> and <a
                href="#" onclick="event.preventDefault();">Privacy Policy</a></span>
          </label>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Terms &amp; Conditions Checkbox</h3>
            <span>Essential</span>
          </div>
          <p>A standard legal agreement checkbox with embedded links and neat alignment.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb10')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb10')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb10" class="code-block" style="display:none;"><code>&lt;label class="terms-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;span class="terms-box"&gt;&lt;i class="fa-solid fa-check"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span class="terms-text"&gt;I agree to the &lt;a href="#"&gt;Terms&lt;/a&gt;&lt;/span&gt;
&lt;/label&gt;</code></pre>
      </div>

      <!-- ============ 11: JELLY BLOB CHECKBOX ============ -->
      <div class="component-card">
        <div class="card-preview">
          <label class="jelly-cb">
            <input type="checkbox" checked>
            <span class="jelly-box"><i class="fa-solid fa-check"></i></span>
            <span class="jelly-text">Interactive Jelly Checkbox</span>
          </label>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Jelly Blob Checkbox</h3>
            <span>Trending</span>
          </div>
          <p>A fun and elastic spring-like jelly checkbox that scales and bounces when clicked.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb11')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb11')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb11" class="code-block" style="display:none;"><code>&lt;label class="jelly-cb"&gt;
  &lt;input type="checkbox" checked&gt;
  &lt;span class="jelly-box"&gt;&lt;i class="fa-solid fa-check"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span class="jelly-text"&gt;Interactive Jelly Checkbox&lt;/span&gt;
&lt;/label&gt;</code></pre>
      </div>

      <!-- ============ 12: NEON PULSING CHECKBOX ============ -->
      <div class="component-card">
        <div class="card-preview">
          <label class="neon-cb">
            <input type="checkbox">
            <span class="neon-box"><i class="fa-solid fa-check"></i></span>
            <span class="neon-text">Neon Cyberpunk Glow</span>
          </label>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Neon Pulsing Checkbox</h3>
            <span>Premium</span>
          </div>
          <p>A vibrant cyan checkbox with an intense glowing state and active outer pulse ripple animation.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb12')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb12')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb12" class="code-block" style="display:none;"><code>&lt;label class="neon-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;span class="neon-box"&gt;&lt;i class="fa-solid fa-check"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span class="neon-text"&gt;Neon Cyberpunk Glow&lt;/span&gt;
&lt;/label&gt;</code></pre>
      </div>

      <!-- ============ 13: RETRO ARCADE CHECKBOX ============ -->
      <div class="component-card">
        <div class="card-preview">
          <label class="arcade-cb">
            <input type="checkbox" checked>
            <span class="arcade-box"></span>
            <span class="arcade-text">Insert Coin</span>
          </label>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Retro Arcade Checkbox</h3>
            <span>New</span>
          </div>
          <p>A retro 8-bit arcade style checkbox utilizing stepping transitions and block pixel checkmarks.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb13')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb13')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb13" class="code-block" style="display:none;"><code>&lt;label class="arcade-cb"&gt;
  &lt;input type="checkbox" checked&gt;
  &lt;span class="arcade-box"&gt;&lt;/span&gt;
  &lt;span class="arcade-text"&gt;Insert Coin&lt;/span&gt;
&lt;/label&gt;</code></pre>
      </div>

      <!-- ============ 14: SLIDE LOCK CHECKBOX ============ -->
      <div class="component-card">
        <div class="card-preview">
          <label class="slidelock-cb">
            <input type="checkbox">
            <div class="slidelock-track">Slide to Lock</div>
            <div class="slidelock-handle">
              <i class="fa-solid fa-angles-right"></i>
            </div>
          </label>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>Slide Lock Checkbox</h3>
            <span>Premium</span>
          </div>
          <p>An interactive sliding track checkbox designed for locking settings or swipe-to-agree interactions.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb14')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb14')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb14" class="code-block" style="display:none;"><code>&lt;label class="slidelock-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;div class="slidelock-track"&gt;Slide to Lock&lt;/div&gt;
  &lt;div class="slidelock-handle"&gt;
    &lt;i class="fa-solid fa-angles-right"&gt;&lt;/i&gt;
  &lt;/div&gt;
&lt;/label&gt;</code></pre>
      </div>

      <!-- ============ 15: 3D DEPTH FLIP CHECKBOX ============ -->
      <div class="component-card">
        <div class="card-preview">
          <label class="depth-cb">
            <input type="checkbox">
            <span class="depth-box"></span>
            <span class="depth-text">Enable 3D Acceleration</span>
          </label>
        </div>
        <div class="card-content">
          <div class="card-top">
            <h3>3D Depth Flip Checkbox</h3>
            <span>Trending</span>
          </div>
          <p>An isometric 3D depth-tilted card checkbox that rotates and flips upon clicking.</p>
          <div class="card-actions">
            <button onclick="toggleCode('cb15')"><i class="fa-solid fa-code"></i> View Code</button>
            <button onclick="copyCode('cb15')"><i class="fa-solid fa-copy"></i> Copy</button>
            <button><i class="fa-solid fa-bookmark"></i> Save</button>
          </div>
        </div>
        <pre id="cb15" class="code-block" style="display:none;"><code>&lt;label class="depth-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;span class="depth-box"&gt;&lt;/span&gt;
  &lt;span class="depth-text"&gt;Enable 3D Acceleration&lt;/span&gt;
&lt;/label&gt;</code></pre>
      </section>

  </div> <!-- main-content -->

  <!-- ================= FOOTER ================= -->
  <footer class="footer">
    <div class="footer-container">

      <div class="footer-col">
        <div class="footer-logo">UIverse</div>
        <p>Build modern, reusable UI components with clean HTML, CSS, and JavaScript.</p>
        <div class="socials">
          <a href="#" target="_blank" title="GitHub"><i class="fab fa-github"></i></a>
          <a href="#" target="_blank" title="LinkedIn"><i class="fab fa-linkedin"></i></a>
          <a href="#" target="_blank" title="Twitter"><i class="fab fa-x-twitter"></i></a>
        </div>
      </div>
<div class="component-card">
  <div class="card-preview">
    <label class="rainbow-cb">
      <input type="checkbox">
      <span class="rainbow-box"><i class="fa-solid fa-check"></i></span>
      <span class="rainbow-text">Celebrate Diversity</span>
    </label>
  </div>
  <div class="card-content">
    <div class="card-top">
      <h3>Rainbow Checkbox</h3>
      <span>Colorful</span>
    </div>
    <p>Animated rainbow gradient fill with smooth transitions when checked.</p>
    <div class="card-actions">
      <button onclick="toggleCode('cb16')"><i class="fa-solid fa-code"></i> View Code</button>
      <button onclick="copyCode('cb16')"><i class="fa-solid fa-copy"></i> Copy</button>
      <button><i class="fa-solid fa-bookmark"></i> Save</button>
    </div>
  </div>
  <pre id="cb16" class="code-block" style="display:none;"><code>&lt;label class="rainbow-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;span class="rainbow-box"&gt;&lt;i class="fa-solid fa-check"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span class="rainbow-text"&gt;Celebrate Diversity&lt;/span&gt;
&lt;/label&gt;</code></pre>
</div>
<div class="component-card">
  <div class="card-preview">
    <label class="music-cb">
      <input type="checkbox">
      <span class="music-box"><i class="fa-solid fa-music"></i>🎵</span>
      <span class="music-text"> Enable Sound Effects</span>
    </label>
  </div>
  <div class="card-content">
    <div class="card-top">
      <h3>Music Note Checkbox</h3>
      <span>Fun</span>
    </div>
    <p>Checkbox with animated music note icon that pulses when selected.</p>
    <div class="card-actions">
      <button onclick="toggleCode('cb17')"><i class="fa-solid fa-code"></i> View Code</button>
      <button onclick="copyCode('cb17')"><i class="fa-solid fa-copy"></i> Copy</button>
      <button><i class="fa-solid fa-bookmark"></i> Save</button>
    </div>
  </div>
  <pre id="cb17" class="code-block" style="display:none;"><code>&lt;label class="music-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;span class="music-box"&gt;&lt;i class="fa-solid fa-music"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span class="music-text"&gt;Enable Sound Effects&lt;/span&gt;
&lt;/label&gt;</code></pre>
</div>
<div class="component-card">
  <div class="card-preview">
    <label class="earth-cb">
      <input type="checkbox">
      <span class="earth-box"><i class="fa-solid fa-earth-americas"></i>🌍</span>
      <span class="earth-text">Eco Mode</span>
    </label>
  </div>
  <div class="card-content">
    <div class="card-top">
      <h3>Earth Checkbox</h3>
      <span>Eco</span>
    </div>
    <p>Eco-friendly themed checkbox with animated globe icon and green glow.</p>
    <div class="card-actions">
      <button onclick="toggleCode('cb18')"><i class="fa-solid fa-code"></i> View Code</button>
      <button onclick="copyCode('cb18')"><i class="fa-solid fa-copy"></i> Copy</button>
      <button><i class="fa-solid fa-bookmark"></i> Save</button>
    </div>
  </div>
  <pre id="cb18" class="code-block" style="display:none;"><code>&lt;label class="earth-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;span class="earth-box"&gt;&lt;i class="fa-solid fa-earth-americas"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span class="earth-text"&gt;Eco Mode&lt;/span&gt;
&lt;/label&gt;</code></pre>
</div>
<div class="component-card">
  <div class="card-preview">
    <label class="lightning-cb">
      <input type="checkbox">
      <span class="lightning-box"><i class="fa-solid fa-bolt"></i>⚡</span>
      <span class="lightning-text">High Performance</span>
    </label>
  </div>
  <div class="card-content">
    <div class="card-top">
      <h3>Lightning Checkbox</h3>
      <span>Power</span>
    </div>
    <p>Dynamic lightning bolt icon with a flash animation when toggled.</p>
    <div class="card-actions">
      <button onclick="toggleCode('cb19')"><i class="fa-solid fa-code"></i> View Code</button>
      <button onclick="copyCode('cb19')"><i class="fa-solid fa-copy"></i> Copy</button>
      <button><i class="fa-solid fa-bookmark"></i> Save</button>
    </div>
  </div>
  <pre id="cb19" class="code-block" style="display:none;"><code>&lt;label class="lightning-cb"&gt;
  &lt;input type="checkbox"&gt;
  &lt;span class="lightning-box"&gt;&lt;i class="fa-solid fa-bolt"&gt;&lt;/i&gt;&lt;/span&gt;
  &lt;span class="lightning-text"&gt;High Performance&lt;/span&gt;
&lt;/label&gt;</code></pre>
</div>



      <div class="footer-col">
        <h3>Explore</h3>
        <ul>
          <li><a href="button.html">Buttons</a></li>
          <li><a href="navbar.html">Navbars</a></li>
          <li><a href="cards.html">Cards</a></li>
          <li><a href="inputs.html">Inputs</a></li>
          <li><a href="forms.html">Forms</a></li>
        </ul>
      </div>

      <div class="footer-col">
        <h3>Resources</h3>
        <ul>
          <li><a href="documentation.html">Documentation</a></li>
          <li><a href="contribute.html">Contribute</a></li>
          <li><a href="#" target="_blank">GitHub Repo</a></li>
          <li><a href="community.html">Community</a></li>
        </ul>
      </div>

      <div class="footer-col">
        <h3>Legal</h3>
        <ul>
          <li><a href="privacypolicy.html">Privacy Policy</a></li>
          <li><a href="terms.html">Terms of Service</a></li>
          <li><a href="license.html">License</a></li>
        </ul>
      </div>

      <div class="footer-col newsletter">
        <h3>Stay Updated</h3>
        <p>Get notified when new components drop.</p>
        <div class="newsletter-form">
          <input type="email" placeholder="your@email.com" />
          <button type="button">Subscribe</button>
        </div>
      </div>

    </div>

    <div class="footer-bottom">
      <p>© 2026 UIverse. Made with ❤️ for developers worldwide.</p>
    </div>
  </footer>

      <script>
        function toggleCode(id) {
          const el = document.getElementById(id);
          if (el) el.style.display = el.style.display === 'none' ? 'block' : 'none';
        }
        function copyCode(id) {
          const el = document.getElementById(id);
          if (!el) return;
          navigator.clipboard.writeText(el.textContent).then(() => alert('Code copied!'));
        }
        function loadTheme() {
  const savedTheme = localStorage.getItem("theme") || "light";
  setTheme(savedTheme, false);
}

function setTheme(theme, persist = true) {
  document.documentElement.setAttribute("data-theme", theme);
  if (persist) localStorage.setItem("theme", theme);
}

function toggleTheme() {
  const currentTheme = document.documentElement.getAttribute("data-theme");
  const newTheme = currentTheme === "light" ? "dark" : "light";
  setTheme(newTheme);
}

window.onload = loadTheme;

    
      </script>
</template>

<script>
export default { name: 'Checkbox' }
</script>
