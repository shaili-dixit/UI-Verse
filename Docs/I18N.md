...
- Runtime locale files are stored in `/src/locales/{lang}.json` and are copied to `dist/locales/` during build.
...
1. Create `/src/locales/{lang}.json` with translation keys.
2. Run `npm run i18n:sync` to copy the locale files into `dist/locales/`.
3. Add an option to the switcher or use the API to switch programmatically:
...
Validate the real translation pipeline with:
